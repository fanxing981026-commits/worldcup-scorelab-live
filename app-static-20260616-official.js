// Daily static snapshot wired for the 2026-06-16 official-source review.
...
function renderSchedule() {
  ...
  const visible = filtered.slice(0, state.scheduleLimit);
  $('#scheduleSummary').textContent = `共 ${filtered.length} 场 · 当前显示 ${visible.length} 场 · 每日数据快照 ${state.modelConfig.predictionFreshness.checkedAt}`;
  $('#scheduleList').innerHTML = visible
    .map((match) => {
      ...
      const hasOfficialResult = Number.isFinite(match.homeScore) && Number.isFinite(match.awayScore);
      const prediction = home && away
        ? predictMatch(home, away, { neutral: true, weights: state.modelConfig.weights })
        : null;
      ...
      return `
        <article class="rounded-2xl border border-white/10 bg-black/18 p-4 transition hover:-translate-y-0.5 hover:border-gold-500/45">
          <div class="flex items-center justify-between gap-3 text-xs font-black">
            ...
            <span class="text-white/42">${match.date} · ${match.kickoffLocal || '时间待定'} MYT</span>
          </div>
          <div class="mt-4 grid grid-cols-[1fr_auto_1fr] items-center gap-3">
            <strong class="text-right">${scheduleTeam(match, 'home')}</strong>
            ${
              hasOfficialResult
                ? `<span class="rounded-xl border border-cyan-400/35 bg-cyan-400/10 px-3 py-2 text-center text-xs font-black text-cyan-400">${match.homeScore}-${match.awayScore}<br>${match.status || 'FT'}</span>`
          ...
          ${hasOfficialResult ? `
            <div class="mt-4 rounded-xl border border-cyan-400/20 bg-cyan-400/[.06] p-3 text-xs font-bold text-white/68">
              <span class="text-cyan-400">官方赛果</span>
              <span class="ml-2">${match.resultSource || match.source}${match.resultCheckedAt ? ` · 检查 ${match.resultCheckedAt}` : ''}</span>
            </div>
          ` : prediction ? `
            <div class="mt-4 rounded-xl border border-white/10 bg-white/[.035] p-3">
              <div class="grid grid-cols-3 gap-2 text-center text-xs font-black">
                <span class="rounded-lg bg-cyan-400/10 px-2 py-2 text-cyan-400">${home.code} 胜 ${percent(prediction.outcome.homeWin)}</span>
                <span class="rounded-lg bg-gold-500/10 px-2 py-2 text-gold-300">平 ${percent(prediction.outcome.draw)}</span>
                <span class="rounded-lg bg-cyan-400/10 px-2 py-2 text-cyan-400">${away.code} 胜 ${percent(prediction.outcome.awayWin)}</span>
              </div>
    ...
    .join('');
  if (!visible.length) {
    $('#scheduleList').innerHTML = '<p class="rounded-2xl border border-white/10 bg-white/[.035] p-5 text-sm font-bold text-white/55 lg:col-span-2">没有符合当前筛选条件的赛程，请清空搜索或切换赛程类型。</p>';
  }
  $('#scheduleMoreBtn').hidden = visible.length >= filtered.length;
}
...
init().catch((error) => {
  ...
  banner.textContent = '数据加载失败，请刷新页面重试。每日数据文件可能暂时无法访问。';
