...
function renderAccuracy() {
  ...
  $('#rankingNext').textContent = `固定快照 · 后续官方更新未纳入`;
...
function renderSchedule() {
  ...
  $('#scheduleSummary').textContent = `共 ${filtered.length} 场 · 当前显示 ${visible.length} 场 · 固定快照 ${state.modelConfig.predictionFreshness.checkedAt}`;
...
init().catch((error) => {
  ...
  banner.textContent = '数据加载失败，请刷新页面重试。固定快照文件可能暂时无法访问。';
